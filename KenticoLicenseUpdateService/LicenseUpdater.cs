...
namespace KenticoLicenseUpdateService
{
    public class LicenseUpdater : ITask
    {
        ...
        public string Execute(TaskInfo task)
        {
            ...
            resultMessage = GenerateNewKeys(generatedKeys, instanceKeys);

            if(generatedKeys.IsEmpty())
            {
                EventLogProvider.LogInformation(nameof(LicenseUpdater), "I", $"Licence key service run finished. Runtime: {stopWatch.Elapsed}, no generated keys");
